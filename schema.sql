-- FRIDGY — paste this entire file into Supabase SQL Editor → Run
...
create table if not exists household_members (
  ...
  name text not null, initials text, sex text, age integer,
  height_cm numeric(5,1), weight_kg numeric(5,1), goal_weight_kg numeric(5,1),
...
create table if not exists pantry_items (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references profiles(id) on delete cascade not null,
  name text not null, qty numeric(10,2) not null, unit text not null,
  cat text, loc text not null default 'fridge', exp date,
  created_at timestamptz not null default now()
);
...
create table if not exists shopping_items (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references profiles(id) on delete cascade not null,
  name text not null, amt text, checked boolean not null default false,
  created_at timestamptz not null default now()
);
...
create table if not exists meal_plans (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references profiles(id) on delete cascade not null,
  day text not null, breakfast text, lunch text, dinner text,
  created_at timestamptz not null default now(),
  unique (user_id, day)
);
